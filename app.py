...
def update_google_status(email, action):
    ...
    try:
        # Update the 'State' tab for real-time tracking
        ...
        # Log event to Task_Logs for permanent audit trail
        logs_sheet.append_row([
            str(uuid.uuid4())[:8], "", 0, email, "", "", 
            now_str, now_str[:10], "", action
        ])
    except Exception as e:
        st.error(f"Sync error: {e}")

# --- 4. SIDEBAR: SHIFT CONTROL ---
...
# Manual Sync to overcome the "Locked Interface" bug
if st.sidebar.button("🔄 Sync with Server"):
    ...
    if not user_row.empty:
        ...
        # Convert string back to datetime object safely
        st.session_state.last_action_time = datetime.strptime(user_row.iloc[0]['Last_Action_Time'], "%m/%d/%Y %H:%M:%S")
    st.rerun()
...
if st.session_state.user_status in ["Login", "End Break"]:
    with st.container(border=True):
        ...
        # Pull question specs from Master
        ...
        if st.button("🚀 Submit Task"):
            ...
            logs_sheet.append_row([
                ...
                worker['Worker_Name'], worker['Role'], now_ist, now_ist[:10], 
                proj_id, task_mode
            ])
